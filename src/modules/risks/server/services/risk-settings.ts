...
export async function getRiskSettings(
  db: PrismaClient,
  tenantId: TenantId,
): Promise<RiskSettingsView> {
  const row = await db.riskSettings.findUnique({
    where: { tenantId },
...
export async function setRiskPrefix(
  ctx: RequestContext,
  input: { prefix: string },
  ...
  return withAuditedTransaction(mctx, async (tx) => {
    const before = await tx.riskSettings.findUnique({
      where: { tenantId: mctx.tenantId },
      select: { prefix: true },
    });
    await tx.riskSettings.upsert({
      where: { tenantId: mctx.tenantId },

...
import { Check, CircleDot, Circle, CheckCircle2, ChevronDown, ChevronRight } from "lucide-react";
 ...
 * activity CTA (absorbs the former Reifegrad/Nächster-Schritt band). Client for
 * the collapse toggle (persisted view preference); all props are serializable
 * plus a server-rendered `actionSlot` node, which passes through fine.
 */
export function EpicLifecycleStepper({ steps, nextStep, actionSlot }: Props) {
  const [collapsed, setCollapsed] = useLocalStorageState("epic:fortschritt:collapsed", false);

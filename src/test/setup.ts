...
import { expectNoRawKeys } from "@/test/no-raw-keys";
...
afterEach(() => {
  // **Erst prüfen, dann aufräumen.** Was dieser Test gerendert hat, darf keinen
  // Katalog-Schlüssel im Klartext zeigen; siehe `no-raw-keys.ts`.
  expectNoRawKeys();
  cleanup();
});

...
export interface CreatePiDialogProps {
...
export function CreatePiDialog({ open, onOpenChange, artId, context }: CreatePiDialogProps) {
  const isControlled = open !== undefined;
  const [selfOpen, setSelfOpen] = useState(false);
  const dialogOpen = open ?? selfOpen;
  const setDialogOpen = (v: boolean) => (isControlled ? onOpenChange?.(v) : setSelfOpen(v));

  const [state, action, isPending] = useActionState(createPiAction, initialState);

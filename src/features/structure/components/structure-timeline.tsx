    ...
    <div className="w-56 shrink-0 space-y-1 border-r p-3">
      ...
      {canCreatePi && (
        <div className="space-y-1.5">
          <CreatePiDialog artId={art.id} />
          <AddStandardPisControl artId={art.id} standards={standards} />
        </div>
      )}
    </div>
  );
...
export function StructureTimeline({
  ...
  const manager = canManageStandards ? (
    <div className="flex justify-end">
      <PiStandardsManager standards={standards} />
    </div>
  ) : null;

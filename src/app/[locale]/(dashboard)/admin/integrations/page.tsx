...
export default async function IntegrationsPage({ searchParams }: Props) {
  ...
  const [jiraConfig, arts] = await Promise.all([
  ...
  const webhookUrl = `${appUrl}/api/integrations/jira/webhook?tenantId=${principal.tenantId}`;
  ...
  return (
    <main className="p-8 max-w-3xl mx-auto space-y-10">
      ...
      <section className="border border-gray-200 rounded-xl p-6 opacity-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Coming soon (PULSE-42)</p>
          </div>
        </div>
      </section>
    </main>

 ...
 * Betriebskosten-Positionen. Editierbar für VS-Owner/Finance/Admin (sonst
 * read-only). Aktive Positionen fließen als Ballot-Kandidaten in jede gestartete
 * Budgeting-Kachel.
 */
export default async function RunTheBusinessPage() {
  ...
  const sections = await Promise.all(
    valueStreams.map(async (vs) => ({
      id: vs.id,
      name: vs.name,
      items: await listRtbItems(db, principal.tenantId, vs.id),
      canManage:
  ...
  return (
    <Page>
      <PageHeader
        ...
        subtitle="Stehende Betriebskosten (Keep the lights on) je Value Stream. Aktive Positionen kommen als Ballot-Kandidaten in jede gestartete Budgeting-Kachel."
      />
        ...
        <div className="space-y-6">
          {sections.map((s) => (
            <section key={s.id} className="space-y-2">
              ...
              <RtbSection valueStreamId={s.id} items={s.items} canManage={s.canManage} />
            </section>
          ))}
        </div>
      )}
